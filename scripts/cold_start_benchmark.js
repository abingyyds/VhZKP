...
async function runColdStartBenchmark() {
    try {
        console.log('Starting cold start performance test...\n');
...
`);
        ...
        for (let i = 0; i < config.testRounds; i++) {
            ...
            if (i === 0) {
                firstTime = timeMs;
                console.log(`First verification time: ${timeMs.toFixed(2)} ms`);
            } else {
                times.push(result.timeNs);
                console.log(`Current verification time: ${timeMs.toFixed(2)} ms`);
            }
        }
        ...
        // Output results
        console.log('\n=== Test Results ===');
        console.log(`First verification time: ${firstTime.toFixed(2)} ms`);
...
if (require.main === module) {
    runColdStartBenchmark().catch(console.error);
} 